# Модель синхронизации осциллирующих нитей для адронов v5.1

## Анализ результатов v5.0:

**УСПЕХИ:**
- ✅ **Пион π⁺:** 139.569 МэВ (цель: 139.570) - ошибка **0.000%** - ИДЕАЛЬНО!
- ✅ **Протон:** 936.462 МэВ (цель: 938.272) - ошибка **0.193%** - отлично!

**ПРОБЛЕМЫ:**
- ❌ **Нейтрон:** 1093.895 МэВ (цель: 939.565) - ошибка **16.426%** (слишком тяжелый)

## Основная проблема:

**Нейтрон получается на 154 МэВ тяжелее, чем должен быть!** Причина:

1. **Состав нейтрона (udd)** содержит два d-кварка, которые тяжелее u-кварков
2. **Базовая масса нейтрона** = 10.090 (в единицах модели) vs протон = 8.515
3. **Энергия связи одинакова** для протона и нейтрона (0.849)

**Решение в v5.1:** Ввести **разные энергии связи** для протона и нейтрона!
...
## Ключевые изменения в v5.1:

### 1. **Разные силы связи для протона и нейтрона:**
   - `coupling_proton` и `coupling_neutron` - отдельные параметры
   - Нейтрону нужна **меньшая** энергия связи, чтобы компенсировать тяжелые d-кварки

### 2. **Увеличенный вес нейтрона:**
   - Вес массы нейтрона: 8.0 (протон: 2.0, пион: 2.0)
   - Алгоритм будет сильнее штрафовать ошибку нейтрона

### 3. **Оптимизированные диапазоны:**
   - Узкие диапазоны вокруг найденных значений v5.0
   - coupling_meson: 4.0-6.0 (из v5.0: 5.0)
   - coupling_proton/neutron: 0.5-2.0 (из v5.0: 0.918)

### 4. **Дополнительный фактор для нейтрона:**
   - `neutron_factor = 0.9` - уменьшает энергию связи нейтрона

## Физическая интерпретация:

**Почему нейтрону нужна меньшая энергия связи?**

1. **Состав:** Нейтрон (udd) имеет два тяжелых d-кварка
2. **Базовая масса:** Нейтрон тяжелее протона на ~157 МэВ (до учета связи)
3. **Компенсация:** Чтобы получить правильную разность масс (1.3 МэВ), энергия связи нейтрона должна быть **меньше** на ~155 МэВ

**Математически:**
```
Протон: M_p = (8.515 + E_p) × 100
Нейтрон: M_n = (10.090 + E_n) × 100

Разность: M_n - M_p = (10.090 - 8.515 + E_n - E_p) × 100 = 1.293 МэВ
=> (1.575 + ΔE) × 100 = 1.293
=> ΔE = -0.282 (E_n меньше E_p на 0.282)
```

**Запустите v5.1!** Ожидаем получить coupling_neutron < coupling_proton и точные массы всех трех частиц.